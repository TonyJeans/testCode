...
<script type="text/javascript">
	...
	function doAssociations(){
		...
		if(rows.length!=1){
			...
			//ajax ,request 定区
			var url_l = "decidedzoneAction_findListNotAssociation.action"
			$.post(url_l,function(data){
			...
			var decidedid =rows[0].id;
			$.post(url_2,{"id":decidedid},function(data){
				
			 if (data=='') return ;

			 	$(data).each(function () {
					$("#associationSelect").append("<option value='"+this.id+"'>"+this.name+"("+this.telephone+")"+"</option");
				});
			},"json"); 
		}
	}
